...
public class FlightController {
    ...
    public String postAddFlight(@Valid AddFlightDto addFlightDto,
        ...
        if (bindingResult.hasErrors()) {
            ...
            redirectAttributes.addFlashAttribute("org.springframework.validation.BindingResult.addFlightDto", bindingResult);
        ...
        List<ViewFlights> flightsList = flight.stream()
                .map(f ->
                        new ViewFlights(f.getId(), f.getFirmOwner().getCompanyName(), f.getFlightFrom(), f.getFlightTo(), f.getTicketPrice(),
                                ...
                                f.getTimeOfFlight().format(DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm")),
                                f.getTicketPrice())
                ).toList();
