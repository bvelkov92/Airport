...
public class FLightControllerIT {
    ...
    @Test
    @WithMockUser(username = "test@test.bg",roles = {"ADMIN"})
    void testRemoveFlight() throws Exception {

        mockMvc.perform(get("/flights/remove/1")
                                .param("id", "1L"))
                .andExpect(status().is3xxRedirection())
                .andExpect(forwardedUrl("redirect:/flights/flight-list"));
...
//
//
//        mockMvc.perform(get("/flights/1").flashAttr("user", user))
//                .andExpect(status().is3xxRedirection())
//                .andExpect(view().name("redirect:/tickets/ticket-list"));
//    }
}