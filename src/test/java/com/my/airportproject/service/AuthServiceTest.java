...
public class AuthServiceTest {
    ...
    @Test
    void testFirmRegister_SaveInvoke() {
        //ARRANGE
        FirmRegisterDto testFirmRegister = new FirmRegisterDto();
        testFirmRegister.setCompanyName(EXIST_COMPANY_NAME);
        testFirmRegister.setUsername(EXIST_USERNAME);
        testFirmRegister.setEmail(EXIST_EMAIL);
        testFirmRegister.setPassword(EXIST_PASSWORD);
        testFirmRegister.setConfirmPassword(EXIST_PASSWORD);

        String ENCODED_PASSWORD = "CryptedPassword";
        when(mockPasswordEncoder.encode(testFirmRegister.getPassword())).thenReturn(ENCODED_PASSWORD);

        //ACT
        authServiceToTest.registerFirm(testFirmRegister);

        //ASSERT

        verify(mockUserRepository).save(userArgumentCaptor.capture());
        User savedUser = userArgumentCaptor.getValue();
        assertEquals(testFirmRegister.getUsername(), savedUser.getUsername());
